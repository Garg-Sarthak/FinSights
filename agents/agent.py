...
def agent(query):
    ...
    6. First call tools, then respond, don't assume that you don't have information, first try calling a tool, in case result is insufficient, only then say that data is insufficient, even in that case, make out what you can from parial data.
    2. If  a user doesn't clearly mentions a section or tool, be smart enough to find the sections from the exhaustive list yourself. You may need to do a multi section analysis, requiring multiple steps, you can't expect user to tell everyhting, be smart.
    ...
    5. Synthesize, Don't Just Report: When you have the final results from your tools, synthesize the information into a clear, concise answer.
    Note : in case data was insufficient, tell the user, that you may want to upload files for this year or maybe company so that I can analyse it.
    """
